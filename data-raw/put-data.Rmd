...
Mop <- function(DT, value.name = "persons", suborder = NULL) {
  ...
  if ("EnglishProficiency" %chin% names(out) &&
      "SpeaksEnglishOnly" %chin% names(out)) {
    out[,
...
NonABS_decoder <-
  ...
  as.data.table
fwrite(NonABS_decoder, "~/Census2016.spec/data-raw/NonABS_decoder.tsv", sep = "\t")
```
...
limit_Age_FamilyComposition_etc <- function(x) {
  if (identical(Region, "SA1") || identical(Region, "SSC")) {
    x[children > 0L]
  } else {
    x
  }
...
freadG(26) %>%
  ...
  .[] %>%
  limit_Age_FamilyComposition_etc %>%
  Mop("children")
```
...
options("Census.Mop.assign.env" = .GlobalEnv)
