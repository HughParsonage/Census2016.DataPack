...
assign(paste0(Region, "__Age_HoursHousekeeping_Sex"), {
  freadG(20) %>%
    drop_cols(grep("Tot", names(.), value = TRUE)) %>%
    drop_cols(grep("^P_", names(.), value = TRUE)) %>%
    melt.data.table(id.vars = Region_key,
...
assign(paste0(Region, "__Age_ProvidedUnpaidDisabilityAssistance_Sex"), {
  freadG(21) %>%
    drop_cols(grep("Tot", names(.), value = TRUE)) %>%
    drop_cols(grep("^P_", names(.), value = TRUE)) %>%
    melt.data.table(id.vars = Region_key,
...
assign(paste0(Region, "__Age_ProvidedUnpaidChildcare_ForOwnChild_ForOtherChild_Sex"), {
  freadG(22) %>%
    drop_cols(grep("Tot", names(.), value = TRUE)) %>%
    drop_cols(grep("^P_", names(.), value = TRUE)) %>%
    melt.data.table(id.vars = Region_key,
...
assign(paste0(Region, "__Age_ChildrenEverBorn"), {
  freadG(24) %>%
    drop_cols(grep("Tot", names(.), value = TRUE)) %>%
    melt.data.table(id.vars = Region_key,
                    value.name = "females") %>%
