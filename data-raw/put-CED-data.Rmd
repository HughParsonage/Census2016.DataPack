...
Mop <- function(DT, value.name = "persons", suborder = NULL) {
  ...
  if (Region %notin% c("SA1", "SA2")) {
    ...
    apparent_population <- sum(as.double(out[[value.name]]))
  }
  ...
  object_name <- 
    paste0(Region, "__", paste0(setdiff(out_noms, c(Region_key, "persons")), collapse = "_"))
  
  decoder <- 
    switch(Region,
           "SA2" = SA2016_decoder[, .(SA2_MAIN16, SA2_NAME16)],
           "SA3" = SA2016_decoder[, .(SA3_CODE16, SA3_NAME16)],
           "SA4" = SA2016_decoder[, .(SA4_CODE16, SA4_NAME16)],
           "STE" = SA2016_decoder[, .(STE_CODE16, STE_NAME16)],
            NULL) %>%
    unique %>%
    setnames(1, Region_key)
  
  decoded_out <- 
    decoder[out, on = Region_key]
  decoded_out[, (Region_key) := NULL]
  
  assign(object_name, decoded_out, envir = .GlobalEnv)
