...
Mop <- function(DT, value.name = "persons", suborder = NULL) {
  ...
  apparent_population <- sum(out[[value.name]])
  permitted_error <- switch(Region,
                            "SA1" = 1.0e6,
                            0.5e6)
  permitted_error <- permitted_error * (1 + any(value.name == "adults" && "MaxSchoolingCompleted" %in% names(DT)))
  ...
  if (AND(!any(c("BornAust", "MaxSchoolingCompleted", "OnlyEnglishSpokenHome") %in% names(DT)),
  ...
  object_name <- 
    paste0(Region, "__", paste0(setdiff(out_noms, c(Region_key, value.name)), collapse = "_"))
...
freadG(16) %>%
  ...
  .[!grepl("schl_ns", variable),
    MaxSchoolingCompleted := gsub("^[MF]_([A-Za-z0-9]+)_.*$", "\\1", variable)] %>%
...
freadG(20) %>%
  ...
  .[grepl("^.*DUDW_([0-9]{1,2})_([0-9]{1,2}).*$", variable), 
    HoursHousekeeping := gsub("^.*DUDW_([0-9]{1,2})_([0-9]{1,2}).*$", "\\1-\\2", variable)] %>%
...
freadG(27) %>%
  ...
  .[grepl("^Intc", variable), FamilyComposition := "Intact"] %>%
  .[grepl("^Step", variable), FamilyComposition := "Stepfamily"] %>%
  .[grepl("^Blnd", variable), FamilyComposition := "Blended"] %>%
  .[grepl("^Otr_cpl", variable), FamilyComposition := "Other couple"] %>%
  .[, ChildrenPresent := !grepl("no_otr_chld_pre", variable)] %>%
  .[] %>%
...
freadG(28) %>%
  ...
  .[, .(families = sum(families)),
    keyby = .(CoupleFamily, HasChildren, IncomeFamily.max)] %>%
  Mop("families")
```
